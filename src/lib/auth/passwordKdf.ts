...
export async function hashPassword(password: string, opts: { iterations?: number } = {}): Promise<PasswordHash> {
  // Default tuned for POS devices (fast enough for login, still strong).
...
export async function verifyPassword(password: string, stored: Pick<PasswordHash, "password_salt" | "password_hash" | "password_iter">) {
  const salt = base64ToBytes(stored.password_salt);
  const expected = base64ToBytes(stored.password_hash);
  const derived = await pbkdf2Sha256(password, salt, clampInt(stored.password_iter ?? 210_000, 120_000, 800_000));
